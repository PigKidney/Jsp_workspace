...
public class ReplyDAO {
	...
	public int add(ReplyDTO dto) {
		...
		try (
			...
			pstmt.setInt(1, dto.getBorard_id());
		...
		try (
			...
			try (ResultSet rs = pstmt.executeQuery()){
				while (rs.next()) {
					...
					dto.setBorard_id(rs.getInt("board_id"));
					...
					dto.setReply_good_count(rs.getInt("reply_good_count"));
					dto.setReply_bad_count(rs.getInt("reply_bad_count"));

...
<% //간편한 방식의 서블릿
	...
	//쿠키는 항상 요청에 포함되어 도착한다(도메인과 path가 맞는 경우에만)
	//쿠키는 하나만 받는 메서드가 없다, 원하는 쿠키 하나만 받는 메서드는 기본적으로는 존재하지 않는다
	...
	for(Cookie cookie : cookies){
		...
		if(cname.equals("age")){
			...
			//요청으로부터 전달받은 쿠키에는 path정보가 없다. 꺼내봤자 null이 나옴.
			...
			//정확하게 같은 쿠키를 쓰기 위해서는 name 뿐만 아니라
			//path값도 일치시켜야한다(path가 다르면 다른 쿠키로 인식한다)
			cookie.setMaxAge(-1); //수명을 음수로 바꾸면 해당 쿠키를 세션으로 설정
			cookie.setPath("/chap09"); //같은쿠키 수정을 위한 path설정
			response.addCookie(cookie);
		}
	}
}
	response.sendRedirect(request.getContextPath()+"/cookie/index.jsp");
	// 수정 시 fav가 따로 추가가 되는 이유는 path가 다르기 때문이다
%>

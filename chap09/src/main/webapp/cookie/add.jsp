...
<%
	...
	Cookie cookie = new Cookie("age","21-50"); //1. 기본 쿠키 (수명설정 안할 경우 브라우저 종료시까지 유지됨)

	//옵션을 다르게 설정해서 쿠키 만들기
	Cookie cookie2 = new Cookie("fav","drama"); //2. 직접 설정한 쿠키
	cookie2.setMaxAge(600); //10분 뒤에 지워지는 쿠키 (기본단위 => 초)
	cookie2.setHttpOnly(true); //자바스크립트에서 이 쿠키를 사용하지 못하도록 한다 (보안상 필요)
	cookie2.setPath("/chap09"); //chap09 전역에서 사용하고싶다, 해당 쿠키가 적용되는 범위를 설정
	cookie2.setValue("comedy"); //값을 변경하는 것은 가능하다(쿠키이름은 변경 불가)

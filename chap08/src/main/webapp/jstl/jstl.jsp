...
<body>
   ...
   <ul>
      ...
      <li>자바의 모든 기능을 사용할 수 있는 것은 아니지만 뷰 페이지를 만들기에는 충분한 기능들이 태그 형식으로 구현되어 있다</li>
   ...
   <c:set var="snack" value="치토스"/> <!-- 스코프 미지정시 page에 어트리뷰트 등록 -->
   
   <c:set var="coffee" value="아메리카노" scope="application"/>
   ...
   <hr />
   <div>c:set - applicationScope : ${applicationScope.snack}</div>
   <div>c:set - sessionScope : ${sessionScope.snack}</div>
   <div>c:set - requestScope : ${requestScope.snack}</div>
   <div>c:set - pageScope : ${pageScope.snack}</div>
   <div>커피 : ${coffee}</div>
   
   <h5># 어트리뷰트에 보관된 값을 활용해 쉽게 if문 사용하기</h5>
   <c:if test="${coffee == '아메리카노'}">
   ...
   <%
      String name = "김철수";
      String[]  coffees = {"Americano", "Cafe Latte", "Americano", "Cafe Mocha", "카라멜 마끼아또"};
   ...
   <c:set var="animals" value="pig/cat#dog/lion/tiger/zebra:elephant"/>

...
<body>
	...
	<dl>
		<dt>1. meta태그의 charset</dt>
			<dd>
				이 페이지를 읽게되는 클라이언트의 웹 브라우저가 
				이 페이지를 해석할 때 사용할 charset 설정
			</dd>
		<dt>2. contentType의 charset</dt>
			<dd>
				이 서블릿에서 응답하는 결과물의 종류 및 charset 설정
			</dd>
		<dt>3. pageEncodeing의 charset</dt>
			<dd>
				.jsp로 작성한 코드를 .java로 변환할 때의 charset
			</dd>
		<dt>4. 현재 파일에서 사용하고 있는 charset</dt>
			<dd>
				우리가 Ctrl + S를 눌러 이클립스를 통해 하드디스크에 저장할 때 사용하는 charset
	...
	<h3># POST 방식 파라미터로 한글을 보냈을 때</h3>
		<ol>
			<li>
				POST 방식 파라미터 값은 요청객체 내부에 실려서 서버에 도착하므로
				URIEncoding의 영향을 받지 않는다
			</li>
			<li>
				request.setCharacterEncoding("EUC-KR")로 요청 객체의 인코딩 방식을 설정하여
				해결할 수 있다
			</li>
		</ol>

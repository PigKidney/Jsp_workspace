...
<body>
	...
	<ul>
		...
		<li>
			요청, 응답 객체를 그대로 다음 서블릿으로 전달하기 때문에 해당 요청에 실려있던 파라미터들도
			그래도 함께 전달된다
		</li>
	</ul>
	...
	<ul>
		...
		<li>
			하나의 요청에 대해 서블릿에서 1차로 자바에 관련된 모든 처리를 마친 후
			포워드를 통해 2차로 .jsp페이지로 넘겨 HTML 코드에 관한 처리를 마치는 방식을 많이 사용한다	
		</li>
		<li>이것을 view와 controller의 분리라고 한다</li>
		<li>forward는 사용하면 실제 view페이지의 위치가 노출되지 않는다
	...
	<ul>
		...
		<li>리다이렉트는 새로운 요청을 다시 보내는 것이기 때문에 이전 요청에 보냈던 파라미터는 기본적으로는 남아있을 수 없다</li>
